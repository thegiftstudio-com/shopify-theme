...
$(".custom_photo_error_txt").remove();
  ...
  return false;
       ...
       var delivery_date_val = $("#delivery-date-5").val();
        ...
        // Added by Velocity: Express Delivery mode + dates (drive cart-level enforcement & the cart delivery note)
        pd_json_property_val["_delivery_mode"] = $("#delivery_mode_selected").val() || 'standard';
...
//velocity: 04-07-2024 Added the function to push the add_to_cart event to GTM
                       ...
                       $("#giftees-number").val("");
                        ...
                        $(".add-to-cart-button").removeAttr("disabled");
                        // Added by Velocity: add/remove the Express charge BEFORE the drawer re-renders.
...
// velocity: 01-06-2025 Updated delivery date and time slot when delivery date and time slot expired.        
      ...
      validateDeliveryDates();
                          // Added by Velocity: refresh the cart-level Express/Standard delivery note
                          if (window.refreshExpressDisplay) { window.refreshExpressDisplay(); }
                        }, 500);
                        // Code end by NR

                        });
                        }); // Added by Velocity: close runExpressEnforcement().then wrapper
