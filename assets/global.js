...
class QuantityInput extends HTMLElement {
  ...
  onButtonClick(event) {
    ...
    validateQtyRules() {
      // Velocity: To skip the auto quantity update to 1 in case of BYOH 
      if (this && this.classList && this.classList.contains('byoh_quantity')) {
          return;
      }
...
class SlideshowComponent extends SliderComponent {
  ...
  onButtonClick(event) {
   ...
   update() {
      ...
      // Get all the slider control buttons
      this.sliderControlButtons = this.querySelectorAll('.slider-counter__link');
      
      // Ensure there are slider control buttons before proceeding
      if (!this.sliderControlButtons.length) return;
  
      // Enable the previous button
      this.prevButton.removeAttribute('disabled');
  
      // Reset all buttons (remove active class and aria-current)
      this.sliderControlButtons.forEach(link => {
          link.classList.remove('slider-counter__link--active');
          link.removeAttribute('aria-current');
      });
  
      // Ensure currentPage is within bounds (1-based index, so subtract 1 for zero-based)
      const currentPageIndex = this.currentPage - 1;
  
      // Check if the currentPageIndex is valid
      if (currentPageIndex >= 0 && currentPageIndex < this.sliderControlButtons.length) {
          const currentButton = this.sliderControlButtons[currentPageIndex];
          
          // Add active class and aria-current to the current page
          currentButton.classList.add('slider-counter__link--active');
...
class VariantSelects extends HTMLElement {
  ...
  updateVariantStatuses() {
    const selectedOptionOneVariants = this.variantData.filter(variant => {
      return this.querySelector(':checked').value === variant.option1.toLowerCase()
    });
